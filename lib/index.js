...
function renderFunction(t, arrowFunction, isEmbeddedReturn) {
    ...
    const declares = properties.map(function(p) {
        return t.variableDeclarator(
            p.value,
            t.memberExpression(t.memberExpression(t.identifier('this'), t.identifier('props')), p.key)
        );
    });
    const destructProps = t.variableDeclaration('const', declares);
    const blocks = [];
    if (!noParam) {
        blocks.push(destructProps);
    }
    ...
    const arrowVisitor = function(state) {
        return function(path) {
            ...
            const isEmbeddedReturn =
                ...
                arrowBody.body[lastIndex].type === 'ReturnStatement' &&
                arrowBody.body[lastIndex].argument.type === 'JSXElement';
